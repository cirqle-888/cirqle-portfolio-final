...
export function FlyerPaperMockup({ page, shape, bend, direction }: {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      try {
        ...
        for (let j = 0; j <= rows; j++) for (let i = 0; i <= cols; i++) {
          ...
          const x = (u - 0.5) * width * 0.86;
          let y = (v - 0.5) * height * 0.86;
          const edge = Math.max(0, (v - 0.6) / 0.4);
          const amplitude = width * bend * (mobile ? 0.72 : 1);
          // Bow, lower-edge roll, and diagonal twist are separate surfaces.
          let z = shape === 0 ? amplitude * Math.pow(2 * u - 1, 2)
            : shape === 1 ? amplitude * Math.sin(v * Math.PI * 1.4)
              : amplitude * (u - 0.5) * (v - 0.5) * 5;
          z += amplitude * edge * edge * (0.65 + u * 0.6);
          y -= amplitude * edge * edge * 0.28;
          const yaw = direction * 0.16, pitch = -0.1;
          ...
          points.push({ x: width / 2 + xx * perspective,
            y: height * 0.5 + yy * perspective, z: depth, u, v });
        }
        ...
        shadow.addColorStop(0, "rgba(49,42,39,0.19)");
        shadow.addColorStop(1, "rgba(49,42,39,0)");
        ...
        if (reflected) {
          ...
          reflected.translate(0, floor * 1.52);
          reflected.scale(1, -0.52);
          ...
          const fade = reflected.createLinearGradient(0, floor, 0, floor + height * 0.32);
          fade.addColorStop(0, "rgba(0,0,0,0.13)"); fade.addColorStop(1, "rgba(0,0,0,0)");
    ...
    schedule();
    return () => { visibility.disconnect(); resize.disconnect(); cancelAnimationFrame(frame); };
  ...
  return <span ref={host} className={`sf-mockup${ready ? " sf-mockup--ready" : ""}`}
    style={{ aspectRatio: `${page.width} / ${page.height * 1.36}` }}>
